...
# Apply script for oneoff Fargate task infrastructure
...
while [ $ATTEMPT -lt $MAX_ATTEMPTS ]; do
    ...
    if aws iam get-role --role-name "hello-fargate-oneoff-ecs-task-role" >/dev/null 2>&1; then
        # Also check the execution role
        if aws iam get-role --role-name "hello-fargate-oneoff-ecs-execution-role" >/dev/null 2>&1; then
