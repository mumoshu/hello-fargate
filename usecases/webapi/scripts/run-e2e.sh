...
# End-to-end test script for webapi use case (ALB JWT validation)
...
TEST_DIR="$PROJECT_ROOT/tests/apitest"
...
TOKEN_ENDPOINT=$(terraform -chdir="$TF_APP_DIR" output -raw cognito_token_endpoint 2>/dev/null)
CLIENT_ID=$(terraform -chdir="$TF_APP_DIR" output -raw cognito_client_id 2>/dev/null)
CLIENT_SECRET=$(terraform -chdir="$TF_APP_DIR" output -raw cognito_client_secret 2>/dev/null)
SCOPE=$(terraform -chdir="$TF_APP_DIR" output -raw cognito_scope 2>/dev/null)

echo "[E2E] INFO: ALB URL: $ALB_URL"
echo "[E2E] INFO: Token Endpoint: $TOKEN_ENDPOINT"
echo "[E2E] INFO: Client ID: $CLIENT_ID"
echo "[E2E] INFO: Scope: $SCOPE"
...
echo "[E2E] INFO: Step 4: Running JWT validation test..."
./apitest \
    -alb-url="$ALB_URL" \
