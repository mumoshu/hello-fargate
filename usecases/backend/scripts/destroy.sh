...
# Destroy script for backend use case
...
terraform -chdir="$TF_APP_DIR" destroy -auto-approve -input=false \
  -var="backend_image_uri=${TF_VAR_backend_image_uri:-dummy}" \
  -var="frontend_image_uri=${TF_VAR_frontend_image_uri:-dummy}" \
  -var="vpc_id=${TF_VAR_vpc_id:-dummy}" \
  -var="ecs_cluster_arn=${TF_VAR_ecs_cluster_arn:-dummy}" || true
