...
resource "aws_security_group" "ecs_sg" {
  name        = "hello-fargate-webapp-ecs-sg"
...
resource "aws_iam_role" "ecs_task_execution_role" {
  name               = "hello-fargate-webapp-ecs-execution-role"
...
resource "aws_iam_role" "ecs_task_role" {
  name               = "hello-fargate-webapp-ecs-task-role"
  assume_role_policy = data.aws_iam_policy_document.ecs_task_assume_role.json
...
resource "aws_ecs_task_definition" "webapp" {
  family                   = "hello-fargate-webapp-task"
  ...
  container_definitions = jsonencode([
    {
      name      = "hello-fargate-webapp-container"
      image     = var.image_uri
      essential = true
...
resource "aws_ecs_service" "webapp" {
  name            = "hello-fargate-webapp-service"
  ...
  load_balancer {
    ...
    container_name   = "hello-fargate-webapp-container"
    container_port   = 8080
  }
