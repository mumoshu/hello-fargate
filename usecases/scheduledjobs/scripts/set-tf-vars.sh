...
# 1. TF_VAR_aws_region
echo "export TF_VAR_aws_region=\"${AWS_REGION}\""

# 2. TF_VAR_image_uri
# Uses AWS_ACCOUNT_ID, AWS_REGION, and IMAGE_TAG (defaults to 'latest')
IMAGE_NAME="fargate-workflow-app" # Should match the name in build.sh
...
# 3. TF_VAR_subnet_ids (expects comma-separated string in TF_SUBNET_IDS)
...
# 5. TF_VAR_prefix
if [[ -n "$TF_PREFIX" ]]; then
    echo "export TF_VAR_prefix=\"${TF_PREFIX}\""
fi

# 6. TF_VAR_schedule_expression
if [[ -n "$TF_SCHEDULE" ]]; then
    echo "export TF_VAR_schedule_expression=\"${TF_SCHEDULE}\""
fi
